...
static const unsigned char code dmp_memory[DMP_CODE_SIZE] = {
    /* bank # 0 */
...
{
    unsigned char xdata gyro_regs[3], accel_regs[3];
    const unsigned char xdata gyro_axes[3] = {DINA4C, DINACD, DINA6C};
    const unsigned char xdata accel_axes[3] = {DINA0C, DINAC9, DINA2C};
    const unsigned char xdata gyro_sign[3] = {DINA36, DINA56, DINA76};
    const unsigned char xdata accel_sign[3] = {DINA26, DINA46, DINA66};
...
{
    long xdata gyro_bias_body[3];
    unsigned char xdata regs[4];
...
#ifdef EMPL_NO_64BIT
    gyro_bias_body[0] = (long)(((float)gyro_bias_body[0] * GYRO_SF) / 1073741824.0f);
    gyro_bias_body[1] = (long)(((float)gyro_bias_body[1] * GYRO_SF) / 1073741824.0f);
    gyro_bias_body[2] = (long)(((float)gyro_bias_body[2] * GYRO_SF) / 1073741824.0f);
#else
...
{
    long xdata accel_bias_body[3];
    unsigned char xdata regs[12];
//    long long accel_sf;
		long xdata accel_sf;
    unsigned short xdata accel_sens;

    mpu_get_accel_sens(&accel_sens);
		accel_sf = (long)accel_sens << 15;
...
{
    const unsigned char xdata regs_end[12] = {DINAFE, DINAF2, DINAAB,
        0xc4, DINAAA, DINAF1, DINADF, DINADF, 0xBB, 0xAF, DINADF, DINADF};
    unsigned short xdata div;
    unsigned char xdata tmp[8];
...
{
    unsigned char xdata tmp[4], accel_fsr;
    float xdata scaled_thresh;
    unsigned short xdata dmp_thresh, dmp_thresh_2;
...
{
    unsigned char xdata tmp[4];
    long xdata thresh_scaled = sf / 1000 * thresh;
...
{
    unsigned char xdata tap, android_orient;

    android_orient = gesture[3] & 0xC0;
    tap = 0x3F & gesture[3];

    if (gesture[1] & INT_SRC_TAP) {
        unsigned char xdata direction, count;
        direction = tap >> 3;
        count = (tap % 8) + 1;
        if (dmp.tap_cb)
...
{
    const unsigned char code regs_continuous[11] =
        {0xd8, 0xb1, 0xb9, 0xf3, 0x8b, 0xa3, 0x91, 0xb6, 0x09, 0xb4, 0xd9};
    const unsigned char code regs_gesture[11] =
...
{
    unsigned char xdata fifo_data[MAX_PACKET_LENGTH];
    unsigned char xdata ii = 0;
