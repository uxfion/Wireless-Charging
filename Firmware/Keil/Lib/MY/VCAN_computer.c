...
#define CMD_IMG     1
    ...
    uart_putbuff(Bluetooth_UART, cmdf, sizeof(cmdf));    //先发送命令

    uart_putbuff(Bluetooth_UART, (uint8_t *)imgaddr, imgsize); //再发送图像

    uart_putbuff(Bluetooth_UART, cmdr, sizeof(cmdr));    //先发送命令
}
...
#define CMD_CCD     2
    ...
    uart_putbuff(Bluetooth_UART, (uint8_t *)ccdaddr, ccdsize); //再发送图像

    uart_putbuff(Bluetooth_UART, cmdr, sizeof(cmdr));    //再发送命令
}
 ...
 */
void vcan_sendware(void *wareaddr, uint16 waresize)
{
#define CMD_WARE     3
