...
void get_ms(unsigned long *count){if(count);}
...
#define i2c_write   IIC_BurstWriteByteToSlave
#define i2c_read    IIC_ReceiveBurstByte
 ...
 */
xdata struct motion_int_cache_s {
...
#if defined MPU6050
xdata const struct gyro_reg_s reg = {
//    .who_am_i       = 0x75,
//    .rate_div       = 0x19,
//    .lpf            = 0x1A,
//    .prod_id        = 0x0C,
...
xdata const struct test_s test = {
//    .gyro_sens      = 32768/250,
//    .accel_sens     = 32768/16,
//    .reg_rate_div   = 0,    /* 1kHz. */
//    .reg_lpf        = 1,    /* 188Hz. */
 ...
 *  @return     0 if successful.
 */
int mpu_read_reg(unsigned char reg, unsigned char *dat)
{
    if (reg == st.reg->fifo_r_w || reg == st.reg->mem_r_w)
        return -1;
    if (reg >= st.hw->num_reg)
        return -1;
    return i2c_read(st.hw->addr, reg, 1, dat);
}
...
int mpu_init(void)
{
   xdata unsigned char  dat[6], rev;
    ...
    if (i2c_read(st.hw->addr, st.reg->accel_offs, 6, dat))
        return -1;
    rev = ((dat[5] & 0x01) << 2) | ((dat[3] & 0x01) << 1) |
        (dat[1] & 0x01);
...
//            log_e("Unsupported software product rev %d.\n", rev);
        ...
        if (i2c_read(st.hw->addr, st.reg->prod_id, 1, dat))
            return -1;
        rev = dat[0] & 0x0F;
        if (!rev) {
 ...
 *  @return     0 if successful.
 */
int mpu_get_gyro_reg(short *dat, unsigned long *timestamp)
{
    unsigned char xdata tmp[6];
 ...
 *  @return     0 if successful.
 */
int mpu_get_accel_reg(short *dat, unsigned long *timestamp)
{
    unsigned char xdata tmp[6];
 ...
 *  @return     0 if successful.
 */
int mpu_get_temperature(long *dat, unsigned long *timestamp)
{
    unsigned char xdata tmp[2];
    short xdata raw;
    ...
    dat[0] = (long)((35 + ((raw - (float)st.hw->temp_offset) / st.hw->temp_sens)) * 65536L);
    return 0;
}
...
int mpu_set_accel_bias(const long *accel_bias)
{
    ...
    short xdata accel_hw[3];
    short xdata got_accel[3];
    short xdata fg[3];
    ...
    if (i2c_read(st.hw->addr, 3, 3, dat))
        return -1;
    fg[0] = ((dat[0] >> 4) + 8) & 0xf;
    fg[1] = ((dat[1] >> 4) + 8) & 0xf;
    fg[2] = ((dat[2] >> 4) + 8) & 0xf;
    ...
    if (i2c_read(st.hw->addr, 0x06, 6, dat))
        return -1;

    got_accel[0] = ((short)dat[0] << 8) | dat[1];
    got_accel[1] = ((short)dat[2] << 8) | dat[3];
    got_accel[2] = ((short)dat[4] << 8) | dat[5];

    accel_hw[0] += got_accel[0];
    accel_hw[1] += got_accel[1];
    accel_hw[2] += got_accel[2];

    dat[0] = (accel_hw[0] >> 8) & 0xff;
    dat[1] = (accel_hw[0]) & 0xff;
    dat[2] = (accel_hw[1] >> 8) & 0xff;
    dat[3] = (accel_hw[1]) & 0xff;
    dat[4] = (accel_hw[2] >> 8) & 0xff;
    dat[5] = (accel_hw[2]) & 0xff;

    if (i2c_write(st.hw->addr, 0x06, 6, dat))
        return -1;
    return 0;
}
...
int mpu_reset_fifo(void)
{
    ...
    if (i2c_write(st.hw->addr, st.reg->int_enable, 1, &dat))
        return -1;
    if (i2c_write(st.hw->addr, st.reg->fifo_en, 1, &dat))
        return -1;
    if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, &dat))
        return -1;

    if (st.chip_cfg.dmp_on) {
        dat = BIT_FIFO_RST | BIT_DMP_RST;
        if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, &dat))
            return -1;
        delay_ms(50);
        dat = BIT_DMP_EN | BIT_FIFO_EN;
...
int mpu_set_gyro_fsr(unsigned short fsr)
{
    ...
    switch (fsr) {
    case 250:
        dat = INV_FSR_250DPS << 3;
        break;
    case 500:
        dat = INV_FSR_500DPS << 3;
        break;
    case 1000:
        dat = INV_FSR_1000DPS << 3;
        break;
    case 2000:
        dat = INV_FSR_2000DPS << 3;
        break;
    default:
        return -1;
    }

    if (st.chip_cfg.gyro_fsr == (dat >> 3))
        return 0;
    if (i2c_write(st.hw->addr, st.reg->gyro_cfg, 1, &dat))
        return -1;
    st.chip_cfg.gyro_fsr = dat >> 3;
    return 0;
}
...
int mpu_set_accel_fsr(unsigned char fsr)
{
    ...
    if (st.chip_cfg.accel_fsr == (dat >> 3))
        return 0;
    if (i2c_write(st.hw->addr, st.reg->accel_cfg, 1, &dat))
        return -1;
    st.chip_cfg.accel_fsr = dat >> 3;
    return 0;
}
...
int mpu_set_lpf(unsigned short lpf)
{
    ...
    if (st.chip_cfg.lpf == dat)
        return 0;
    if (i2c_write(st.hw->addr, st.reg->lpf, 1, &dat))
        return -1;
    st.chip_cfg.lpf = dat;
    return 0;
}
...
int mpu_set_sample_rate(unsigned short rate)
{
    ...
    if (st.chip_cfg.dmp_on)
        ...
        dat = 1000 / rate - 1;
        if (i2c_write(st.hw->addr, st.reg->rate_div, 1, &dat))
            return -1;

        st.chip_cfg.sample_rate = 1000 / (1 + dat);
...
int mpu_set_sensors(unsigned char sensors)
{
   xdata unsigned char  dat;
    ...
    st.chip_cfg.clk_src = dat & ~BIT_SLEEP;
    ...
    if (!(sensors & INV_XYZ_ACCEL))
        dat |= BIT_STBY_XYZA;
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_2, 1, &dat)) {
        st.chip_cfg.sensors = 0;
        return -1;
    }
        ...
        unsigned char *sensors, unsigned char *more)
{
    ...
    unsigned char xdata dat[MAX_PACKET_LENGTH];
    unsigned char xdata packet_size = 0;
    unsigned short xdata fifo_count, index = 0;
    ...
    if (i2c_read(st.hw->addr, st.reg->fifo_count_h, 2, dat))
        return -1;
    fifo_count = (dat[0] << 8) | dat[1];
    if (fifo_count < packet_size)
        return 0;
//    log_i("FIFO count: %hd\n", fifo_count);
    if (fifo_count > (st.hw->max_fifo >> 1)) {
        /* FIFO is 50% full, better check overflow bit. */
        if (i2c_read(st.hw->addr, st.reg->int_status, 1, dat))
            return -1;
        if (dat[0] & BIT_FIFO_OVERFLOW) {
            mpu_reset_fifo();
            return -2;
        }
    }
    ...
    if (i2c_read(st.hw->addr, st.reg->fifo_r_w, packet_size, dat))
        return -1;
    more[0] = fifo_count / packet_size - 1;
    sensors[0] = 0;

    if ((index != packet_size) && st.chip_cfg.fifo_enable & INV_XYZ_ACCEL) {
        accel[0] = (dat[index+0] << 8) | dat[index+1];
        accel[1] = (dat[index+2] << 8) | dat[index+3];
        accel[2] = (dat[index+4] << 8) | dat[index+5];
        sensors[0] |= INV_XYZ_ACCEL;
        index += 6;
    }
    if ((index != packet_size) && st.chip_cfg.fifo_enable & INV_X_GYRO) {
        gyro[0] = (dat[index+0] << 8) | dat[index+1];
        sensors[0] |= INV_X_GYRO;
        index += 2;
    }
    if ((index != packet_size) && st.chip_cfg.fifo_enable & INV_Y_GYRO) {
        gyro[1] = (dat[index+0] << 8) | dat[index+1];
        sensors[0] |= INV_Y_GYRO;
        index += 2;
    }
    if ((index != packet_size) && st.chip_cfg.fifo_enable & INV_Z_GYRO) {
        gyro[2] = (dat[index+0] << 8) | dat[index+1];
 ...
 */
int mpu_read_fifo_stream(unsigned short length, unsigned char *dat,
    unsigned char *more)
{
    unsigned char xdata tmp[2];
    unsigned short xdata fifo_count;
    if (!st.chip_cfg.dmp_on)
        return -1;
    if (!st.chip_cfg.sensors)
        return -1;
    ...
    if (i2c_read(st.hw->addr, st.reg->fifo_r_w, length, dat))
        return -1;
...
{
    unsigned char xdata tmp[4], shift_code[3], ii;
...
{
    int xdata jj, result = 0;
    float xdata st_shift[3], st_shift_cust, st_shift_var;
...
{
    int xdata jj, result = 0;
    unsigned char xdata tmp[3];
    float xdata st_shift, st_shift_cust, st_shift_var;
...
{
    unsigned char xdata tmp[6];
    unsigned char xdata tries = 10;
    int xdata result = 0x07;
    short xdata dat;
...
{
    unsigned char xdata dat[MAX_PACKET_LENGTH];
    unsigned char xdata packet_count, ii;
    unsigned short xdata fifo_count;

    dat[0] = 0x01;
    dat[1] = 0;
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 2, dat))
        return -1;
    delay_ms(200);
    dat[0] = 0;
    if (i2c_write(st.hw->addr, st.reg->int_enable, 1, dat))
    ...
    if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, dat))
        return -1;
    dat[0] = BIT_FIFO_RST | BIT_DMP_RST;
    if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, dat))
        return -1;
    delay_ms(15);
    dat[0] = st.test->reg_lpf;
    if (i2c_write(st.hw->addr, st.reg->lpf, 1, dat))
        return -1;
    dat[0] = st.test->reg_rate_div;
    if (i2c_write(st.hw->addr, st.reg->rate_div, 1, dat))
        return -1;
    if (hw_test)
        dat[0] = st.test->reg_gyro_fsr | 0xE0;
    else
        dat[0] = st.test->reg_gyro_fsr;
    if (i2c_write(st.hw->addr, st.reg->gyro_cfg, 1, dat))
        return -1;

    if (hw_test)
        dat[0] = st.test->reg_accel_fsr | 0xE0;
    else
        dat[0] = test.reg_accel_fsr;
    if (i2c_write(st.hw->addr, st.reg->accel_cfg, 1, dat))
        return -1;
    if (hw_test)
        delay_ms(200);
    ...
    dat[0] = INV_XYZ_GYRO | INV_XYZ_ACCEL;
    if (i2c_write(st.hw->addr, st.reg->fifo_en, 1, dat))
        return -1;
    ...
    for (ii = 0; ii < packet_count; ii++) {
        short xdata accel_cur[3], gyro_cur[3];
        if (i2c_read(st.hw->addr, st.reg->fifo_r_w, MAX_PACKET_LENGTH, dat))
            return -1;
        accel_cur[0] = ((short)dat[0] << 8) | dat[1];
        accel_cur[1] = ((short)dat[2] << 8) | dat[3];
        accel_cur[2] = ((short)dat[4] << 8) | dat[5];
        accel[0] += (long)accel_cur[0];
        accel[1] += (long)accel_cur[1];
        accel[2] += (long)accel_cur[2];
        gyro_cur[0] = (((short)dat[6] << 8) | dat[7]);
        gyro_cur[1] = (((short)dat[8] << 8) | dat[9]);
        gyro_cur[2] = (((short)dat[10] << 8) | dat[11]);
    ...
    if (1) {
        accel[0] = (long)(((float)accel[0]*65536.0f) / test.accel_sens /
            packet_count);
        accel[1] = (long)(((float)accel[1]*65536.0f) / test.accel_sens /
            packet_count);
        accel[2] = (long)(((float)accel[2]*65536.0f) / test.accel_sens /
            packet_count);
...
{
#ifdef MPU6050
    const unsigned char xdata tries = 2;
    long xdata gyro_st[3], accel_st[3];
    unsigned char xdata accel_result, gyro_result;
#ifdef AK89xx_SECONDARY
...
#endif
    int xdata result;
    unsigned char xdata accel_fsr, fifo_sensors, sensors_on;
    unsigned short xdata gyro_fsr, sample_rate, lpf;
    unsigned char xdata dmp_was_on;
        ...
        unsigned char *dat)
{
    ...
    if (i2c_write(st.hw->addr, st.reg->bank_sel, 2, tmp))
        return -1;
    if (i2c_write(st.hw->addr, st.reg->mem_r_w, length, dat))
        return -1;
    return 0;
}
        ...
        unsigned char *dat)
{
    ...
    if (i2c_write(st.hw->addr, st.reg->bank_sel, 2, tmp))
        return -1;
    if (i2c_read(st.hw->addr, st.reg->mem_r_w, length, dat))
        return -1;
    return 0;
}
...
{
    unsigned short xdata ii;
    unsigned short xdata this_write;
    /* Must divide evenly into st.hw->bank_size to avoid bank crossings. */
#define LOAD_CHUNK  (16)
    unsigned char xdata cur[LOAD_CHUNK], tmp[2];
 ...
 *  @return     0 if successful.
 */
int mpu_get_compass_reg(short *dat, unsigned long *timestamp)
{
	if(dat);
	if(timestamp);
#ifdef AK89xx_SECONDARY
    ...
    unsigned char lpa_freq)
{
    ...
    if (lpa_freq) {
        ...
        dat[0] = thresh_hw;
        dat[1] = time;
        if (i2c_write(st.hw->addr, st.reg->motion_thr, 2, dat))
            goto lp_int_restore;

        /* Force hardware to "lock" current accel sample. */
        delay_ms(5);
        dat[0] = (st.chip_cfg.accel_fsr << 3) | BITS_HPF;
        if (i2c_write(st.hw->addr, st.reg->accel_cfg, 1, dat))
            goto lp_int_restore;
        ...
        int xdata ii;
        char xdata *cache_ptr = (char*)&st.chip_cfg.cache;
